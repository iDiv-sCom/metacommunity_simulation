...
for(scenario in 1:8) {
sim.data = sim.list[[scenario]]
data = sim.data[[1]]
coords = sim.data[[2]]
coords$patch = 1:200
data = data %>% left_join(coords, by = "patch")
coords = data %>% select(x, y)
...
surviving_species = colSums(Y) > 0
Y = Y[,surviving_species]
PA = apply(Y, 2, function(P) ifelse(P>mean(P), 1, 0))
...
spatial_temporal = data_wo_burnin[data_wo_burnin$patch %in% 1:100,]
...
X_corrected = cbind(X[-c(1:100),], Y[-c(10001:10100),]) %>% select(-time, -patch)
...
# Train model - Poisson
spatial_temporal_poisson = sjSDM(Y[-c(1:100),] %>% select(-time, -patch) %>% as.matrix(), 
...
spatial_temporal = spatial_temporal[spatial_temporal$time %in% 1001:1101,]
X = spatial_temporal %>% select(env, time, x, y)
PA = data.frame(apply(Y, 2, function(P) ifelse(P>mean(P), 1, 0)))
colnames(PA) = colnames(Y)

X_corrected = cbind(X[-c(1:100),], PA[-c(10001:10100),]) %>% select(-time, -patch)
X_corrected = cbind(X_corrected[,c(1:3)], scale(X_corrected[,-c(1:3)])+0.0001)
...
spatial_temporal_PA= sjSDM(PA[-c(1:100),] %>% select(-time, -patch) %>% as.matrix(), 
                           linear(X_corrected, form), 
                           spatial = linear(coords, ~ x + y + x:y + I(x^2) + I(y^2)),
                           family = binomial(),
                           sampling = 100L)
...
saveRDS(list(results_spatial, results_temporal, results_spatial_temporal), file = paste0("results/01_prelim_results_",scenario,".Rds"))
}